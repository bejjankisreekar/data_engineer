...
**Apache Hadoop** is the open-source framework (2006) that made big data practical. It applies [distributed computing](Distributed_Computing.md) to two problems at once:
...
- Classic [master–slave architecture](Master_Slave_Architecture.md): NameNode = master, DataNodes = workers.
...
- HDFS → cloud data lakes ([Azure Data Lake Storage](../03_Data_Storage/Azure_Data_Lake_Storage.md), S3)
...
Understanding Hadoop is understanding *why* modern tools look the way they do — see [Big_Data_Evolution_Timeline.md](Big_Data_Evolution_Timeline.md).
...
The ResourceManager doesn't run jobs — per application it launches an **ApplicationMaster** (in a container) which then negotiates containers for that job's tasks. RM stays lightweight; this is [master delegation](Master_Slave_Architecture.md) in action.
...
|---|---|
| HDFS data | [ADLS Gen2](../03_Data_Storage/Azure_Data_Lake_Storage.md) (distcp/ADF copy) |
...
| Oozie workflows | [Data Factory](../04_ETL_ELT/Azure_Data_Factory.md) / Databricks Workflows |
