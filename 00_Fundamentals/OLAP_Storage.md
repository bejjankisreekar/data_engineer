...
Where [OLTP](OLTP_Storage.md) serves the *application*, OLAP serves the *analyst*.
...
This is exactly why analytical file formats like [Parquet](../02_File_formats/Parquet.md) are columnar.
...
See [ETL vs ELT](../04_ETL_ELT) and [Data_Lake_vs_Warehouse_vs_Database.md](../03_Data_Storage/Data_Lake_vs_Warehouse_vs_Database.md) for how data travels from OLTP to OLAP.
...
Columnar engines (and formats like [Parquet](../02_File_formats/Parquet.md)/ORC) layer several tricks:
...
- **The lakehouse** collapses "lake + warehouse" into one: Delta/Iceberg tables on the lake give ACID + schema enforcement, and a SQL engine on top serves BI ([Big_Data_Evolution_Timeline.md](Big_Data_Evolution_Timeline.md)).
