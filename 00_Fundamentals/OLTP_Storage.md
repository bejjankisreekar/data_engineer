...
(Contrast this with [OLAP storage](OLAP_Storage.md), which stores data column by column.)
...
Full detail on the other side: [OLAP_Storage.md](OLAP_Storage.md).
...
OLTP systems are usually the **source** of a data engineer's pipelines: the business runs on them, and we copy their data out for analytics — see [Data_Lake_vs_Warehouse_vs_Database.md](../03_Data_Storage/Data_Lake_vs_Warehouse_vs_Database.md).
...
Finding any row among 100 million takes **3–4 page reads** instead of millions. This is why point lookups are milliseconds. (Deep dive: [SQL_Indexes.md](../01_SQL/SQL_Indexes.md).)
...
Result: updates touch one place; no risk of two rows disagreeing about a customer's address. The price: reads need joins — which is exactly why [OLAP](OLAP_Storage.md) denormalizes.
