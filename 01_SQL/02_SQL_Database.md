...
New to SQL entirely? Start with [What is SQL](01_What_is_SQL.md) first — it introduces the language itself before this file goes deeper into what a relational database is used for. See the [Glossary](../GLOSSARY.md) for quick definitions of any of these words, and [SQL Keys and Joins](07_SQL_Keys_and_Joins.md) for how multiple tables connect to each other.
...
- Supports relationships (see [Keys and Joins](07_SQL_Keys_and_Joins.md))
- Supports joins
- Fast querying
...
For those cases, see [SQL Warehouse](13_SQL_Warehouse.md) and [Data Lake vs Warehouse vs Database](../03_Data_Storage/01_Data_Lake_vs_Warehouse_vs_Database.md).
...
- **Transaction log (WAL)** — every change is written to a sequential log before data files, giving crash recovery and feeding **CDC** tools that stream changes into the lake ([01_OLTP_Storage.md](../00_Fundamentals/01_OLTP_Storage.md) covers this in depth).
...
- **Isolation levels** (Read Committed → Serializable) trade correctness anomalies against throughput — detailed table in [01_OLTP_Storage.md](../00_Fundamentals/01_OLTP_Storage.md), commands in [12_SQL_DCL_TCL.md](12_SQL_DCL_TCL.md).
...
3. **Watch for hard deletes and schema drift** — watermarks miss deletes ([soft-delete/CDC discussion](../00_Fundamentals/01_OLTP_Storage.md)); a dropped column breaks tomorrow's load. Schema-drift alerts belong in the pipeline.
...
| Wide-column | Cassandra | Massive write throughput, [masterless](../00_Fundamentals/04_Master_Slave_Architecture.md) |
...
- **Auto-growth events** on data/log files cause mysterious latency spikes — pre-size files in on-prem SQL Server; managed services handle this for you (part of what you pay for — [02_SaaS_PaaS_IaaS.md](../05_cloud/02_SaaS_PaaS_IaaS.md)).
