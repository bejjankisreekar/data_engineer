...
Azure SQL Database, Synapse Analytics, and Databricks (via Spark SQL) all support standard `INSERT`/`UPDATE`/`DELETE`. In [ETL/ELT pipelines](../04_ETL_ELT/01_ETL_vs_ELT.md), DML is frequently generated automatically by tools like [Azure Data Factory](../04_ETL_ELT/02_Azure_Data_Factory.md) rather than typed by hand — a pipeline might insert thousands of new rows nightly as part of a scheduled load.
