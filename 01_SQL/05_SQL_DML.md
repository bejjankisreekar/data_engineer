...
Just like `UPDATE`, a `DELETE` without a `WHERE` clause removes every row in the table (though, unlike [`TRUNCATE`](04_SQL_DDL.md), it does so one row at a time and can typically still be undone if caught before the transaction is saved — see [12_SQL_DCL_TCL.md](12_SQL_DCL_TCL.md)).
...
Azure SQL Database, Synapse Analytics, and Databricks (via Spark SQL) all support standard `INSERT`/`UPDATE`/`DELETE`. In [ETL/ELT pipelines](../05_ETL_ELT/01_ETL_vs_ELT.md), DML is frequently generated automatically by tools like [Azure Data Factory](../05_ETL_ELT/02_Azure_Data_Factory.md) rather than typed by hand — a pipeline might insert thousands of new rows nightly as part of a scheduled load.
...
2. **Delete-then-insert scoped to the batch** — `DELETE WHERE load_date = '2026-07-19'` then insert that day (atomic inside a [transaction](12_SQL_DCL_TCL.md)).
...
(Plus a pause between batches on busy systems.) Better still: design so mass deletes become **partition drops/switches** ([DDL](04_SQL_DDL.md)) or Delta `replaceWhere` — metadata operations instead of row carnage.

## Soft deletes and the audit trail

Many systems never physically DELETE: an `is_deleted BIT + deleted_at` flag preserves history, keeps foreign keys intact, and — critically for data engineering — makes deletes **visible to incremental extracts** (a hard-deleted row simply vanishes from a watermark query; see [OLTP extraction](../00_Fundamentals/01_OLTP_Storage.md)). Costs: every query needs `WHERE is_deleted = 0` (hide it in a [view](10_SQL_Views.md)), unique constraints need filtering, and GDPR "right to erasure" still requires a real purge path.
...
- On Delta, many small MERGEs = many small files + version bloat: batch micro-changes, then `OPTIMIZE` ([Spark_Processing.md](../07_PySpark/Spark_Processing.md)).
