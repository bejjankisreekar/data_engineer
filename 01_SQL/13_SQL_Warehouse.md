...
> Deep foundations for this note live in [02_OLAP_Storage.md](../00_Fundamentals/02_OLAP_Storage.md) (columnar internals, star schema mechanics, SCDs). This part focuses on the warehouse as a *system you design and load*.
...
An MPP warehouse spreads each table across many compute nodes ([distributed computing](../00_Fundamentals/03_Distributed_Computing.md)); *how* you spread it decides join cost:
...
The [lakehouse](../00_Fundamentals/06_Big_Data_Evolution_Timeline.md) (Delta + Databricks SQL / Fabric) now covers most classic warehouse duties. What tips the decision:
...
- **Concurrency cliffs** — one warehouse sized for the nightly load serving 500 analysts at 9am; fix with workload isolation (separate compute for load vs BI — the [storage/compute separation](../00_Fundamentals/02_OLAP_Storage.md) dividend).
...
- Everything in [OLAP physical design](../00_Fundamentals/02_OLAP_Storage.md) — partitioning, clustering, file sizing — applies verbatim.
