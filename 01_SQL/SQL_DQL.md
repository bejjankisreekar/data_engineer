...
Also non-sargable: leading-wildcard `LIKE '%son'` (no seek possible; `LIKE 'J%'` is fine) and [implicit type conversions](SQL_Data_Types.md). This single habit fixes more slow queries than any server setting.
...
- **Top-N per group** — `ROW_NUMBER() ... WHERE rn <= 3` (via subquery/CTE, since WHERE can't see window results — [processing order](What_is_SQL.md)).
...
- `SELECT *` in production breaks contracts silently (new columns flow downstream), defeats [columnar pruning](../00_Fundamentals/OLAP_Storage.md), and inflates network/memory. Exploration only.
- `DISTINCT` slapped on to "fix duplicates" hides a join bug and adds a full sort/hash — find the row multiplication instead ([joins](SQL_Keys_and_Joins.md)).
