...
Why: `id NOT IN (1, 2, NULL)` unfolds to `id<>1 AND id<>2 AND id<>NULL`; the last is *unknown*, poisoning the whole predicate ([three-valued logic](What_is_SQL.md)). The professional default for anti-joins:
...
Nuanced answer that distinguishes seniors: modern optimizers **decorrelate** many correlated subqueries into joins/aggregations automatically — the two spellings often produce identical plans. But decorrelation has limits (complex correlations, some engines' apply operators), and when it fails you get once-per-row execution: fine for 100 outer rows, catastrophic for 100 million. The pro workflow: write for clarity, **check the plan** ([reading plans](SQL_DQL.md)), and rewrite to window functions or pre-aggregated CTE joins when you see per-row apply on a big input. Window functions replace the classic correlated patterns wholesale:
