...
- **Transaction log (WAL)** — every change is written to a sequential log before data files, giving crash recovery and feeding **CDC** tools that stream changes into the lake ([01_OLTP_Storage.md](../../01_Foundations/Fundamentals/01_OLTP_Storage.md) covers this in depth).
...
- **Isolation levels** (Read Committed → Serializable) trade correctness anomalies against throughput — detailed table in [01_OLTP_Storage.md](../../01_Foundations/Fundamentals/01_OLTP_Storage.md), commands in [12_SQL_DCL_TCL.md](12_SQL_DCL_TCL.md).
...
3. **Watch for hard deletes and schema drift** — watermarks miss deletes ([soft-delete/CDC discussion](../../01_Foundations/Fundamentals/01_OLTP_Storage.md)); a dropped column breaks tomorrow's load. Schema-drift alerts belong in the pipeline.
...
| Wide-column | Cassandra | Massive write throughput, [masterless](../../01_Foundations/Fundamentals/04_Master_Slave_Architecture.md) |
