...
- **Distributed engines add partial aggregation**: each node pre-aggregates locally, then shuffles only the compact partials — why `groupBy().sum()` in Spark ships tiny data but `groupByKey()`/collect-style patterns ship everything ([shuffle mechanics](../../03_Programming/PySpark/Spark_Processing.md)).
