...
TCL guarantees *your* changes commit atomically; **isolation levels** decide how much of *other* concurrent transactions you observe ([anomaly table](../../01_Foundations/Fundamentals/01_OLTP_Storage.md)):
...
Delta Lake gives ACID per table via an [optimistic-concurrency transaction log](../../06_Programming/PySpark/Why_Spark_Why_Databricks.md): writers prepare files, then attempt to commit a new log version; a conflicting concurrent commit fails one writer (`ConcurrentAppendException`) who must retry. Key differences from a database engine:
