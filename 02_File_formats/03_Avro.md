...
- Timestamp logical types: `timestamp-millis` vs `-micros` mismatches between producer libs and Spark readers shift or truncate times silently ([timezone discipline](../01_SQL/03_SQL_Data_Types.md)).
...
- *Why Avro for Kafka but Parquet for the lake?* Write-optimized whole-record row format + registry-governed evolution in flight; read-optimized columnar at rest ([row vs column](../00_Fundamentals/01_OLTP_Storage.md)).
