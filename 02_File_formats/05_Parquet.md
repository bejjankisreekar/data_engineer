...
- **File size**: target 100 MB–1 GB; thousands of KB-size files = footer-reading overhead dominating scans (the [small files problem](../00_Fundamentals/05_Hadoop_Architecture.md) reborn).
...
- **Types**: write proper DECIMAL/timestamp logical types, not strings — "numbers as strings in Parquet" wastes the format ([type mapping tax](../01_SQL/03_SQL_Data_Types.md)).
