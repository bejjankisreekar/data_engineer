...
Avro schemas are JSON documents; the data itself is compact binary (no field names repeated per record — unlike [JSON](JSON.md)):
...
Avro owns the **in-flight** leg; it stops being the storage format at the first Delta write ([Parquet](Parquet.md) columnar wins at rest). The legacy pattern of landing `.avro` files in the lake ("Event Hubs Capture") persists in many estates — treat those as bronze input, convert promptly; querying raw Avro folders analytically pays row-format prices on every scan.
...
- Timestamp logical types: `timestamp-millis` vs `-micros` mismatches between producer libs and Spark readers shift or truncate times silently ([timezone discipline](../01_SQL/SQL_Data_Types.md)).
...
- *Why Avro for Kafka but Parquet for the lake?* Write-optimized whole-record row format + registry-governed evolution in flight; read-optimized columnar at rest ([row vs column](../00_Fundamentals/OLTP_Storage.md)).
