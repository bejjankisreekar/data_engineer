...
- **`explode`** turns arrays into rows (grain change! — row counts multiply, [same math as joins](../01_SQL/SQL_Keys_and_Joins.md)); `explode_outer` keeps parents with empty arrays.
- **`from_json`/`to_json`** parse JSON *strings inside columns* — the standard first step for Kafka/Event Hub payloads.
- Deep schemas: define once with `StructType`, or store the DDL string; never let inference guess event schemas in production (same reasoning as [CSV](CSV.md)).
...
The hybrid pattern pros converge on: **promote the queried-hot fields to typed columns; keep the full payload in a variant/string column** for the unforeseen question ([data types note](../01_SQL/SQL_Data_Types.md)).
...
- Compressed `.json.gz` from vendors: same splittability trap as [CSV](CSV.md) — many medium files beat one giant one.
