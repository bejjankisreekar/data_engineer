...
- Huge catalog of ready services (databases, [data lakes](../04_Data_Storage/03_Azure_Data_Lake_Storage.md), Databricks, AI)
...
Modern data platforms are overwhelmingly **public cloud**: ADLS, [Databricks](../07_PySpark/Why_Spark_Why_Databricks.md), Data Factory, Synapse are all public-cloud services. But large enterprises (banks, healthcare, insurance) are often **hybrid** — expect pipelines that pull from on-prem SQL Servers (via self-hosted integration runtimes) into the cloud lake.
...
- **Self-hosted Integration Runtime** — the data engineer's daily hybrid tool: a small agent installed *inside* the private network that lets [Data Factory](../05_ETL_ELT/02_Azure_Data_Factory.md) reach on-prem SQL Servers without opening inbound firewall holes (it dials *out*).
