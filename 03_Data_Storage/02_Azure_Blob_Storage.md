...
1. **Entra ID + RBAC roles** (`Storage Blob Data Reader/Contributor`) granted to **managed identities** of ADF/Databricks — no secrets to leak ([identity as perimeter](../05_cloud/02_SaaS_PaaS_IaaS.md)).
...
Plus network posture: private endpoints + default-deny firewall on any account holding real data ([cloud network posture](../05_cloud/01_Public_Private_Hybrid_Cloud.md)).
...
- **Egress awareness**: cross-region reads from your "cheap" storage add per-GB fees ([data transfer economics](../05_cloud/01_Public_Private_Hybrid_Cloud.md)) — keep compute in the storage region.
