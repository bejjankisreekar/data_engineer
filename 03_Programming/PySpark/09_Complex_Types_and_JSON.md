...
Real-world data is nested — API payloads, event streams, order-with-line-items documents ([JSON background](../../05_Storage_and_Formats/File_Formats/02_JSON.md)). PySpark handles nesting natively with three types: **struct** (named fields), **array** (ordered list), **map** (key→value).
