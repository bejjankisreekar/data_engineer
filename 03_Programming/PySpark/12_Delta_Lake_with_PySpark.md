...
2. **Idempotency check**: re-running the same MERGE with the same source converges (same end state) — which makes retries safe ([the property that matters](../../06_Data_Engineering/ETL_ELT/01_ETL_vs_ELT.md)).
...
- **Change Data Feed** (`delta.enableChangeDataFeed = true`): read row-level changes between versions (`table_changes('t', 12, 15)`) — build incremental downstream loads without diffing ([CDC concepts](../../06_Data_Engineering/ETL_ELT/01_ETL_vs_ELT.md)).
