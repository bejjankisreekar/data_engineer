...
- [Azure Data Lake Storage](../03_Data_Storage/03_Azure_Data_Lake_Storage.md) (raw storage)
...
| **Self-hosted IR (SHIR)** | An agent *you install* inside a private network | On-prem/VNet-locked sources — it dials **out**, so no inbound firewall holes ([hybrid connectivity](../05_cloud/01_Public_Private_Hybrid_Cloud.md)) |
...
- **All secrets in Key Vault**, referenced by linked services; ADF's **managed identity** granted narrowly on storage/SQL ([identity as perimeter](../05_cloud/02_SaaS_PaaS_IaaS.md)) — connection strings pasted into linked services are a security-review finding.
