...
1. **Entra ID + RBAC roles** (`Storage Blob Data Reader/Contributor`) granted to **managed identities** of ADF/Databricks — no secrets to leak ([identity as perimeter](../../03_Cloud/Cloud_Concepts/02_SaaS_PaaS_IaaS.md)).
...
Plus network posture: private endpoints + default-deny firewall on any account holding real data ([cloud network posture](../../03_Cloud/Cloud_Concepts/01_Public_Private_Hybrid_Cloud.md)).
...
- **Cost telemetry**: per-container inventory reports + cost tags; the classic surprise is transaction (per-operation) costs from millions of tiny writes, not GB — another push against [small files](../../06_Programming/PySpark/Spark_Processing.md).
- **Egress awareness**: cross-region reads from your "cheap" storage add per-GB fees ([data transfer economics](../../03_Cloud/Cloud_Concepts/01_Public_Private_Hybrid_Cloud.md)) — keep compute in the storage region.
