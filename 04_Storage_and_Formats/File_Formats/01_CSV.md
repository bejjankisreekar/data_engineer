...
Plain CSV compresses ~80–90% with gzip — but a `.csv.gz` file is **not splittable**: one 10 GB gzip = one Spark task reading it alone ([parallelism](../../06_Programming/PySpark/Spark_Processing.md) gone). Options:
...
3. **Keep the raw file immutable** in the bronze/landing zone — you *will* need to re-parse with fixed options ([medallion](../../05_Data_Engineering/ETL_ELT/01_ETL_vs_ELT.md)).
