...
- Preserve partition layouts (`year=2026/...`) during conversion so downstream pruning survives; revisit [file sizing](../../06_Programming/PySpark/Spark_Processing.md) — Hive-era small files should be compacted in the same pass.
...
- Stats-based skipping only works if data is **sorted/clustered** on the filtered column within files — random layout = min/max ranges that never exclude anything (same reason [Z-ordering](../../06_Programming/PySpark/Spark_Processing.md) exists for Parquet/Delta).
