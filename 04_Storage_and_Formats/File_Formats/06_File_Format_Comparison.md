...
- **Reconcile counts and checksums** across the boundary (rows in = rows out + quarantined; [SQL instrumentation](../../02_Databases/SQL/08_SQL_Aggregate_Functions.md)).
- **Resolve types explicitly** — CSV/JSON's stringly-typed values become real DECIMAL/TIMESTAMP here, per declared schema, never by inference ([type mapping tax](../../02_Databases/SQL/03_SQL_Data_Types.md)).
- **Keep the raw** — bronze retains the original bytes; conversions are re-runnable, so parser bugs are recoverable ([medallion](../../05_Data_Engineering/ETL_ELT/01_ETL_vs_ELT.md)).
- Make the conversion [idempotent](../../02_Databases/SQL/05_SQL_DML.md) — the same input file re-processed must not double its rows.
...
- The pipeline example above says "a single Parquet file" — at real scale that's an anti-pattern; write *appropriately sized multiple* files per partition ([file sizing](../../06_Programming/PySpark/Spark_Processing.md)).
