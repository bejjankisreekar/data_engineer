...
- [Azure Data Lake Storage](../04_Data_Storage/03_Azure_Data_Lake_Storage.md) (raw storage)
...
| **Self-hosted IR (SHIR)** | An agent *you install* inside a private network | On-prem/VNet-locked sources — it dials **out**, so no inbound firewall holes ([hybrid connectivity](../03_Cloud/01_Public_Private_Hybrid_Cloud.md)) |
...
- **All secrets in Key Vault**, referenced by linked services; ADF's **managed identity** granted narrowly on storage/SQL ([identity as perimeter](../03_Cloud/02_SaaS_PaaS_IaaS.md)) — connection strings pasted into linked services are a security-review finding.
...
- Track **rows copied vs rows expected** per run in the control table ([reconciliation habit](../01_SQL/13_SQL_Warehouse.md)) — a green Copy that moved 0 rows is a data incident wearing a success icon.
...
- ForEach default parallelism + a target database = accidental DoS on the source; set batch count deliberately ([connection limits](../01_SQL/02_SQL_Database.md)).
...
- Copy Activity's implicit type mapping (`String` → everything) quietly forfeits [type fidelity](../01_SQL/03_SQL_Data_Types.md) — define explicit mappings or land to Parquet with a declared schema.
