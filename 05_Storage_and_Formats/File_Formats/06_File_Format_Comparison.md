...
- **Keep the raw** — bronze retains the original bytes; conversions are re-runnable, so parser bugs are recoverable ([medallion](../../06_Data_Engineering/ETL_ELT/01_ETL_vs_ELT.md)).
...
- The pipeline example above says "a single Parquet file" — at real scale that's an anti-pattern; write *appropriately sized multiple* files per partition ([file sizing](../../03_Programming/PySpark/Spark_Processing.md)).
