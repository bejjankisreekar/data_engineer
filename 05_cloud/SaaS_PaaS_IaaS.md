...
[Deployment models](Public_Private_Hybrid_Cloud.md) answered *whose computers*. Service models answer a different question: **how much of the stack do you manage vs the provider?**
...
- **Azure examples:** Azure SQL Database, [Azure Data Factory](../04_ETL_ELT/Azure_Data_Factory.md), Azure Databricks, Synapse, App Service, [ADLS](../03_Data_Storage/Azure_Data_Lake_Storage.md)
...
A typical Azure data platform is **almost entirely PaaS** — that's the sweet spot: you focus on pipelines and data, Microsoft runs the machinery. (It's also the Databricks pitch in [Why_Spark_Why_Databricks.md](../06_PySpark/Why_Spark_Why_Databricks.md): Spark-on-IaaS-you-manage vs Spark-as-a-platform.)
...
- "Serverless" ≠ "costless": pay-per-TB-scanned engines make an unpartitioned `SELECT *` a billing event ([OLAP gotchas](../00_Fundamentals/OLAP_Storage.md)).
