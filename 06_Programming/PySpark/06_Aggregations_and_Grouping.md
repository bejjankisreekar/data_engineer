...
Same concepts as [SQL aggregation](../../02_Databases/SQL/08_SQL_Aggregate_Functions.md) — `groupBy` = GROUP BY, and every SQL rule (null behavior, WHERE-vs-HAVING) carries over. What's new is the distributed cost model underneath.
...
Nulls are ignored by aggregates exactly as in SQL — `F.count("col")` vs `F.count("*")` differ by the null count ([details](../../02_Databases/SQL/08_SQL_Aggregate_Functions.md)).
...
Same semantics as [SQL ROLLUP/CUBE](../../02_Databases/SQL/08_SQL_Aggregate_Functions.md).
...
- **Pre-aggregate before joins**: `orders.groupBy("cust").agg(...)` *then* join to customers ships far less than join-then-aggregate ([join grain](../../02_Databases/SQL/07_SQL_Keys_and_Joins.md)) — the single most common query rewrite in tuning sessions.
- **Aggregates as data-quality checks** — the [instrumentation habit](../../02_Databases/SQL/08_SQL_Aggregate_Functions.md) in PySpark form:
...
- **Re-aggregatability**: store `SUM` and `COUNT` in summary tables, never `AVG` — averages of averages lie ([why](../../02_Databases/SQL/08_SQL_Aggregate_Functions.md)).
