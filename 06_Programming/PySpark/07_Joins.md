...
Join *logic* is identical to [SQL joins](../../02_Databases/SQL/07_SQL_Keys_and_Joins.md) — grain, fan-out, null keys, all of it transfers. What PySpark adds: syntax details that cause duplicate-column pain, and the **distributed cost model** (broadcast vs shuffle) that decides whether your join takes seconds or hours.
...
Semi/anti never duplicate rows (no fan-out possible) and carry no right-side columns — prefer them over inner-join-then-distinct or [NOT IN traps](../../02_Databases/SQL/09_SQL_Subqueries.md).
...
Rows with null join keys match *nothing* — even other nulls (`null == null` is not true; [three-valued logic](../../02_Databases/SQL/01_What_is_SQL.md)). In a left join they survive with null right-side columns; in an inner join they vanish silently. Decide explicitly: filter them, default them (`coalesce(key, lit(-1))` to an Unknown member — [warehouse pattern](../../02_Databases/SQL/13_SQL_Warehouse.md)), or `eqNullSafe` if null-matches-null is truly intended.
...
A left join that *grows* the left side means the right side wasn't unique on the key ([fan-out](../../02_Databases/SQL/07_SQL_Keys_and_Joins.md)) — dedupe the dimension first ([window pattern](08_Window_Functions.md)).
