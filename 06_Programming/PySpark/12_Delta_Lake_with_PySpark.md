...
2. **Idempotency check**: re-running the same MERGE with the same source converges (same end state) — which makes retries safe ([the property that matters](../../05_Data_Engineering/ETL_ELT/01_ETL_vs_ELT.md)).
...
`OPTIMIZE` fixes [small files](Spark_Processing.md) from streaming/frequent MERGEs; Z-ORDER clusters a high-cardinality filter column for data skipping ([why](../../01_Foundations/Fundamentals/02_OLAP_Storage.md)). Newer Databricks: **liquid clustering** (`CLUSTER BY`) replaces both partitioning and Z-order for most new tables, and **predictive optimization** runs OPTIMIZE/VACUUM for you.
...
- **Change Data Feed** (`delta.enableChangeDataFeed = true`): read row-level changes between versions (`table_changes('t', 12, 15)`) — build incremental downstream loads without diffing ([CDC concepts](../../05_Data_Engineering/ETL_ELT/01_ETL_vs_ELT.md)).
- **Deletion vectors**: DELETE/UPDATE mark rows instead of rewriting whole files — massive win for GDPR deletes on big tables.
- **Column mapping**: enables rename/drop column without rewriting data ([DDL note](../../02_Databases/SQL/04_SQL_DDL.md)).
- **UniForm**: expose a Delta table with Iceberg metadata for cross-engine readers ([format war détente](../../01_Foundations/Fundamentals/06_Big_Data_Evolution_Timeline.md)).
...
- History is not a backup: `RESTORE` can't survive a VACUUM'd version and `_delta_log` corruption is real — critical tables still get [cross-account protection](../../04_Storage_and_Formats/Data_Storage/02_Azure_Blob_Storage.md).

## Checkpoint

1. Implement a daily [SCD2](../../02_Databases/SQL/13_SQL_Warehouse.md) dimension load with MERGE (expire changed rows, insert new versions).
