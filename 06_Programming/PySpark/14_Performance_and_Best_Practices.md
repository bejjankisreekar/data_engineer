...
1. **Read less**: filter early on partition/cluster columns, select only needed columns, [sargable predicates](../../02_Databases/SQL/06_SQL_DQL.md) so [Parquet pushdown](../../04_Storage_and_Formats/File_Formats/05_Parquet.md) works.
...
Unit-test transformations with tiny inline DataFrames; integration-test pipelines against sample files; data-quality-test *production data* with [expectations at the boundaries](../../05_Data_Engineering/ETL_ELT/01_ETL_vs_ELT.md). `pyspark` installs from pip — CI runs all of this on a plain build agent.
...
- Watermarks/state committed atomically with the data ([ETL gotchas](../../05_Data_Engineering/ETL_ELT/01_ETL_vs_ELT.md)).
...
Job clusters + auto-terminate + spot workers as the default ([cost levers](Why_Spark_Why_Databricks.md)); Photon for SQL-shaped work, not UDF-heavy work; `availableNow` triggers instead of 24/7 streams unless latency pays for itself ([13](13_Structured_Streaming.md)); tag jobs and review the cost dashboard monthly — a data platform's bill is an engineering artifact, not an act of God ([FinOps](../../03_Cloud/Cloud_Concepts/01_Public_Private_Hybrid_Cloud.md)).

### The senior checklist for any new pipeline

1. Contract: schema declared, drift policy chosen, owner named ([03](03_Schemas_and_Data_Types.md), [JSON drift](../../04_Storage_and_Formats/File_Formats/02_JSON.md)).
2. Idempotent writes + retry-safe orchestration.
3. Quality gates with quarantine + alert, not silent drops ([ETL](../../05_Data_Engineering/ETL_ELT/01_ETL_vs_ELT.md)).
