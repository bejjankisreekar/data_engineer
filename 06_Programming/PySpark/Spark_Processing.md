...
- **Predicate pushdown** — if you filter after reading a [Parquet](../../04_Storage_and_Formats/File_Formats/05_Parquet.md) file, Spark pushes the filter *into the read* and skips whole files/row-groups.
...
- *Why is Spark fast?* In-memory processing + lazy evaluation with whole-plan optimization + massive parallelism. (vs [MapReduce](../../01_Foundations/Fundamentals/05_Hadoop_Architecture.md) writing to disk every stage.)
...
- **Partitioned writes:** `df.write.partitionBy("year","month")` = pruning for readers; never partition by high-cardinality columns ([OLAP physical design](../../01_Foundations/Fundamentals/02_OLAP_Storage.md)).
...
- Timezones: Spark session timezone silently rewrites timestamps on read/write — pin `spark.sql.session.timeZone` and store UTC ([data type pitfalls](../../02_Databases/SQL/03_SQL_Data_Types.md)).
