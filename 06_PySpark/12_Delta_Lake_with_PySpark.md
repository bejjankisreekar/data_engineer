...
The [upsert](../01_SQL/SQL_DML.md), Delta-style — the single most-run statement in lakehouse pipelines:
...
2. **Idempotency check**: re-running the same MERGE with the same source converges (same end state) — which makes retries safe ([the property that matters](../04_ETL_ELT/ETL_vs_ELT.md)).
...
`OPTIMIZE` fixes [small files](Spark_Processing.md) from streaming/frequent MERGEs; Z-ORDER clusters a high-cardinality filter column for data skipping ([why](../00_Fundamentals/OLAP_Storage.md)). Newer Databricks: **liquid clustering** (`CLUSTER BY`) replaces both partitioning and Z-order for most new tables, and **predictive optimization** runs OPTIMIZE/VACUUM for you.
...
- **Change Data Feed** (`delta.enableChangeDataFeed = true`): read row-level changes between versions (`table_changes('t', 12, 15)`) — build incremental downstream loads without diffing ([CDC concepts](../04_ETL_ELT/ETL_vs_ELT.md)).
- **Deletion vectors**: DELETE/UPDATE mark rows instead of rewriting whole files — massive win for GDPR deletes on big tables.
- **Column mapping**: enables rename/drop column without rewriting data ([DDL note](../01_SQL/SQL_DDL.md)).
- **UniForm**: expose a Delta table with Iceberg metadata for cross-engine readers ([format war détente](../00_Fundamentals/Big_Data_Evolution_Timeline.md)).
...
- History is not a backup: `RESTORE` can't survive a VACUUM'd version and `_delta_log` corruption is real — critical tables still get [cross-account protection](../03_Data_Storage/Azure_Blob_Storage.md).

## Checkpoint

1. Implement a daily [SCD2](../01_SQL/SQL_Warehouse.md) dimension load with MERGE (expire changed rows, insert new versions).
