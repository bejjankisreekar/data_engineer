...
- **Exactly-once, spelled out**: replayable source (Kafka offsets / file lists) + checkpoint (what's been read) + transactional sink (Delta) = end-to-end exactly-once *for the default single-sink path*. Every deviation (foreachBatch to two places, non-transactional sinks) drops you to at-least-once — design [idempotent](../05_ETL_ELT/01_ETL_vs_ELT.md) anyway.
...
- Small files: streaming's [signature failure mode](12_Delta_Lake_with_PySpark.md) — enable auto-compaction or scheduled OPTIMIZE from day one, and keep checkpoint folders out of lifecycle/tiering policies ([storage gotcha](../04_Data_Storage/03_Azure_Data_Lake_Storage.md)).
