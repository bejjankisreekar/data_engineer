...
A warehouse doesn't generate its own data — it's filled by pipelines pulling from operational ([OLTP](../00_Fundamentals/01_OLTP_Storage.md)) systems and external sources. See [ETL vs ELT](../05_ETL_ELT/01_ETL_vs_ELT.md) and [Data Pipelines](../05_ETL_ELT/03_Data_Pipelines.md) for how that loading actually happens.
...
- **Skipping the staging layer** to "save a step" removes your only safe place to re-run a broken transformation without re-hitting the source system — see the [raw retention discipline](../05_ETL_ELT/01_ETL_vs_ELT.md) this maps to.
