...
- Wire ADF **diagnostic logs to Log Analytics** and set **Azure Monitor alerts** on failed runs or SLA misses — see [Monitoring & Observability](../12_Monitoring_and_Observability/00_Monitoring_Learning_Path.md).
...
ADF integrates with **Git** (Azure Repos/GitHub): authoring happens in a branch, publishing generates **ARM templates**, and a release pipeline promotes dev→test→prod with per-environment parameters. Covered in [CI/CD](../07_DevOps/Git_GitHub/09_Production_Best_Practices_and_CICD.md) and [DataOps](../14_Testing_and_DataOps/00_Testing_and_DataOps_Learning_Path.md).
