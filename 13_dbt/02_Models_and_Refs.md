...
On the first run it builds everything; on later runs, `is_incremental()` is true and it processes only new rows, MERGE-ing on `unique_key`. This is dbt's answer to the same incremental-load problem you solved manually in [Project 1](../18_Projects/02_Project_1_Batch_Medallion_Pipeline.md).
