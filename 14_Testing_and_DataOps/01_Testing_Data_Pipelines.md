...
For SQL transformations, **dbt tests** ([dbt tests](../13_dbt/03_Tests_and_Documentation.md)) cover the *data*; for the *logic*, dbt supports **unit tests** (fixed input rows → expected output) so you can verify a model's SQL against known cases — the SQL-world equivalent of the chispa example above.
