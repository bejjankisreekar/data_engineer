...
| **Monitor & observe** | Operational + data health in production ([monitoring](../12_Monitoring_and_Observability/00_Monitoring_Learning_Path.md)) |
...
Tools make this concrete: dbt **targets**, Databricks **workspaces/catalogs per env**, ADF **git integration + ARM parameters** ([ADF](../11_Orchestration/02_ADF_Orchestration.md)), and [Terraform](../Job%20Interviews/Terraform/Terraform%20Interview%20Questions.md) to provision identical infra per environment. `ref()` in dbt and parameterization in ADF make the *same code* run against different environments — the whole point.
