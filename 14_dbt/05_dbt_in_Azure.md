...
| **Airflow** | `BashOperator`/`DbtCloudRunJobOperator`/**Cosmos** — dbt as tasks in a DAG ([Airflow](../12_Orchestration/04_Apache_Airflow.md)) |
| **ADF** | Run dbt in a container/Batch or trigger dbt Cloud via its API ([ADF](../12_Orchestration/02_ADF_Orchestration.md)) |
| **Databricks Workflows** | A native **dbt task** type runs your project as a job task ([Workflows](../12_Orchestration/03_Databricks_Workflows.md)) |

The clean mental model from [Orchestration](../12_Orchestration/01_Orchestration_Fundamentals.md) holds: the orchestrator **schedules**, dbt **transforms**.
