...
- **Incremental refresh** — only reload recent partitions, not the whole dataset (the same [incremental](../14_dbt/02_Models_and_Refs.md) idea, in Power BI).
- **Refresh failures** need monitoring like any [pipeline](../13_Monitoring_and_Observability/00_Monitoring_Learning_Path.md) — a failed refresh means stale dashboards.

Coordinating Gold-load completion → Power BI refresh is an orchestration concern engineers own ([Orchestration](../12_Orchestration/00_Orchestration_Learning_Path.md)).
